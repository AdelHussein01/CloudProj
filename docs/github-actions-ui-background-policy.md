...
- `yellow` waits for approval through the `ui-yellow-approval` GitHub Environment.
- Any other existing design background is treated as neutral and passes.

The check reads `apps/web/app/globals.css` and evaluates the final `body` `background` or `background-color` declaration.

## Required GitHub Setting For Yellow Approval

Create a protected environment:

1. Open the repository on GitHub.
2. Go to `Settings` -> `Environments`.
3. Create an environment named `ui-yellow-approval`.
4. Enable `Required reviewers`.
5. Add the teacher, repository admin, or chosen reviewer.
6. Save the environment.

After this is configured, a commit that changes the body background to yellow will pause the workflow until a reviewer approves it.
...
Adding this pauses for approval:
